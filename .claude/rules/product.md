...
| **Recommendation** | A predicted practice to improve next, with a score from 0–1; the primary flow returns exactly two for an eligible team-month, or an explanatory empty list when fewer than two practices remain |
...
| **Publicly recommendable team-month** | The team has a snapshot in a valid global prediction month, a usable earlier baseline, and ≥2 candidate practices below max maturity; no future outcome is required |
...
- **Public recommendation history:** The team must have a snapshot in the requested valid prediction month and a usable baseline snapshot strictly before it
