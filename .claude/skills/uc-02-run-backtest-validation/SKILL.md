...
description: Rolling window validation of the global two-month adaptive blend, split into primary and sensitivity results. Use when modifying the backtest tab or per-month results display. No user-adjustable model parameters exist.
---
...
3. Loading spinner displayed; button disabled during run
...
- **A scope has zero qualifying months** (e.g. too few prediction months have closed a full outcome window yet): that section shows "Not enough completed months to report this scope" rather than a misleading 0%
...
## Cross-references
- **Related Domain Skills:** `/domain-validation` (`BacktestEngine` algorithm, primary/sensitivity split, random and time-aware-popularity baseline formulas), `/domain-ml` (`PolicyEngine` owns the cohort, monthly selection, and scoring the backtest replays), `/domain-api` (backtest route), `/domain-frontend` (backtest tab rendering)
