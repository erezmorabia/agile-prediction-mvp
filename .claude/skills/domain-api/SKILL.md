...
- Backtest endpoint uses `asyncio.get_event_loop().run_in_executor()` — do not convert to `await asyncio.to_thread()` without testing concurrent request handling
...
| `create_routes()` | `src/api/routes.py` | `create_app()` | `service: APIService` → `APIRouter` with all 9 routes registered |
...
| `APIService.get_team_months()` | `src/api/service.py` | `GET /api/teams/{team_name}/months` | `team_name` → `list[int]` (team-recorded valid prediction months with a usable earlier baseline) or `None` |
...
| `APIService.run_backtest()` | `src/api/service.py` | `POST /api/backtest` (via executor) | no params → `{per_month_results, primary, sensitivity}` |
